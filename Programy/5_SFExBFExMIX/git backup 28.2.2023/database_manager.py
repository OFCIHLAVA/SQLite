...
def update_database_bom_from_cq_file(database_path=str, cq_bom_file_path=str, progress_callback=None): # This will open the txt CQ bom file and add the items in it with their final monument to database programy, table dilyXboudy
    ...
    if is_txt_file(cq_bom_file_path):
        ...
        if not is_txt_file_empty((cq_bom_file_path)):
            ...
            start_time = time.time()
            sqllite.insert_many_records(database_path, "dilyXboudy", dily_monumenty_pridat, [], progress_callback)
            end_time = time.time()
            trvani = end_time - start_time
            ...
            if final_monuments_pridat:
                ...
                inserted_monuments = sqllite.insert_many_records(database_path, 'boudyXprogramy', final_monuments_pridat, [], progress_callback)
            ...
            if final_pns_pridat:
                ...
                inserted_pns = sqllite.insert_many_records(database_path, 'part_numbers', final_pns_pridat, ["part number"], progress_callback)
                print(inserted_pns)

            # Measure total time taken
            total_end_time = time.time()
            total_trvani = total_end_time - total_start_time

            return f'OK - BOM Database succesfully updated. Time took: {round(total_trvani,2)}s.'
...
def update_database_bom_from_txt_list(database_path=str, txt_list_path=str, progress_callback=None): # This will open the txt file with list of itemXin_monument and add the items in it with their final monument to database programy, table dilyXboudy. Predpoklada se zahlavi na prvni lince v souboru.
        ...
        if not is_txt_file_empty((txt_list_path)):
            ...
            with open(txt_list_path, "r", encoding = "UTF-8") as f:
                ...
                for line in dily_monumenty_pridat:
                    print(line)
                    if len(line) != 2:
                        return f'Warning!\nData in selected file appears to have invalid format.\nNothing added. Please check the file and try again...'
            # vzit tento seznam a updatovat databazi tabulku dilxbouda
            print(f'Vkladam data do databaze, this may take a while...')
            start_time = time.time()
            sqllite.insert_many_records(database_path, "dilyXboudy", dily_monumenty_pridat, [], progress_callback)
            ...
            # A. vytvorit set pridavanych monumentu a pro kazdy proverit, jestli uz je v databazi monumentu a pokud jeste ne → pridat ho tam.
            # pridavane dily monumnetu jsou ve formatu (pn, monument)
            print(f'proveruji, zda se vsechny monumenty z pridavaneho txt souboru uz nachazi v databazi. Pokud ne → pridavam je.')
            pridavane_monumenty = {pair[1] for pair in dily_monumenty_pridat}
            print(f'{len(pridavane_monumenty)} unikatnich monumentu v txt souboru')
            pridavane_monumenty = list(pridavane_monumenty)            
            uz_drive_pridane_monumenty = check_if_monuments_in_database(database_path, pridavane_monumenty)
            if uz_drive_pridane_monumenty:
                print(f'z toho {len(uz_drive_pridane_monumenty)} monumetnu uz v databazi je z drivejska.')
            ...
            print(f'Nakonec tedy budu pridavat {len(final_monuments_pridat)} unikatnich monumentu z txt souboru')
            # Insert these into database with default values        
            if final_monuments_pridat:
                # add monuments
                print(f'These monuments not yet in database → inserting: {final_monuments_pridat} with default values "?"')
            ...
            # B. vytvorit set pridavanych part numberu a pro kazdy proverit, jestli uz je v databazi part numberu a pokud jeste ne → pridat ho tam.
            # pridavane dily monumnetu jsou ve formatu (pn, monument)
            print(f'proveruji, zda se vsechny part numbery z pridavaneho txt souboru uz nachazi v databazi. Pokud ne → pridavam je.')
            pridavane_pns = {pair[0] for pair in dily_monumenty_pridat}
            print(f'{len(pridavane_pns)} unikatnich PN v txt souboru')
            pridavane_pns = list(pridavane_pns)
            uz_drive_pridane_pns = check_if_pns_in_database(database_path, pridavane_pns)
            print(f'z toho {len(uz_drive_pridane_pns)} PN uz v databazi je z drivejska.')

            # Creare result list of tuples of part numbers and their default values to be added, which are not yet in database. (list of tuples needed for sql query inserting the records)
            if uz_drive_pridane_pns:
                final_pns_pridat = [(pn, "?") for pn in pridavane_pns if pn not in uz_drive_pridane_pns]
            else:
                final_pns_pridat = [(pn, "?") for pn in pridavane_pns]            
            

            print(f'Nakonec tedy budu pridavat {len(final_pns_pridat)} unikatnich PNS z txt souboru')
...
def add_many_monumnets(database_path=str, monuments_to_add_file_path=str, progress_callback=None): # This will open the txt file with list of itemXin_monument and add the items in it with their final monument to database programy, table dilyXboudy. Predpoklada se zahlavi na prvni lince v souboru.
    # Check if selected path is valid (not empty, is text file and there are some data in file)
    # Path leads to txt file → next check
    if is_txt_file(monuments_to_add_file_path):
        ...
        if not is_txt_file_empty((monuments_to_add_file_path)):
            ...
            print(f'Vkladam data do databaze, this may take a while...')
            start_time = time.time()
            sqllite.insert_many_records(database_path, "boudyXprogramy", monumenty_pridat, ["final monument"], progress_callback)
            end_time = time.time()
            trvani = end_time - start_time
            print(f'Zaznamy vlozeny do databaze. Time took: {trvani}')
            ...
            return f'OK - Monuments Database succesfully updated.\nTime took: {round(total_trvani,2)}s.'
        return f'Warning! Selected .txt file is empty. Nothing to add...'
    return f'Warning! The selected filepath does not lead to a .txt file.\n Please, chose a different path...'
...
def update_many_monuments(database_path=str, update_file_path=str): # Function to change many monuments via GUI + txt file. (program / rules etc)
    ...
    if is_txt_file(cq_bom_file_path):
        ...
        if not is_txt_file_empty((cq_bom_file_path)):
            ...
            return f'OK - Monuments Database succesfully updated. Time took: {round(total_trvani,2)}s.'
        return f'Warning! Selected .txt file is empty. Nothing to add...'
    return f'Warning! The selected filepath does not lead to a .txt file. Please, chose a different path...'
...
def delete_many_monuments(database_path=str, delete_monuments_file_path=str, progress_callback=None): # Function to delete many monuments from database, monuments specified in txt file. 
    ...
    if is_txt_file(delete_monuments_file_path):
        ...
        if not is_txt_file_empty((delete_monuments_file_path)):
            ...
            with open(delete_monuments_file_path, "r", encoding = "UTF-8") as f:
                monumenty_delete = [tuple([field.strip().replace("\n", "") for field in line.split('\t')]) for line in f if line.strip()]
            
            zahlavi_dat = monumenty_delete[0]
            # Odstraneni zahlavi ze samotnych dat
            monumenty_delete.pop(0)
            print(f'Zahlavi souboru: {zahlavi_dat}')
            print(f's dulpikatz {len(monumenty_delete)}')
            # Odstraneni duplicitnich linek
            monumenty_delete = list(set(monumenty_delete))
            print(f'set {len(monumenty_delete)}')
            print(f'Pocet unikatnich radek v soubory s daty: {len(monumenty_delete)}')
            ...
            # Deleting monuments
            print(f'Deleting {len(monumenty_delete)} monuments')
            start_time = time.time()
            deleted_monuments = sqllite.delete_many_records(database_path, "boudyXprogramy", ["final monument"], monumenty_delete, progress_callback)
            end_time = time.time()
            trvani = end_time - start_time 
            
            # Deleting their BOM records
            if deleted_monuments:            
                print(f'Deleted {len(deleted_monuments)} monuments. Trvani: {trvani} s')
                print(f'Step 1 OK')
                print(f'Also deleting BOM pairs of these monuments.')
                start_time = time.time()
                deleted_monuments_bom_pairs = sqllite.delete_many_records(database_path, "dilyXboudy", ["obsazen v"], monumenty_delete, progress_callback)
                end_time = time.time()
                trvani = end_time - start_time 
                if deleted_monuments_bom_pairs:
                    print(f'Deleted {len(deleted_monuments)} BOM records. Trvani: {trvani} s')
                    print(f'Step 2 OK')
                else:
                    print(f'NO BOM pairs to deleted!.')           
            else:
                print(f'NOTHING deleted!.')           
                progress_callback(1, 1)
            if not deleted_monuments:
                return f'WARNING! No record matches given criteria.\nNothing deleted.'
            
            # Measure total time taken
            total_end_time = time.time()
            total_trvani = total_end_time - total_start_time

            return f'OK - Monuments {len(deleted_monuments)} succesfully deleted.\nAlso deleted BOM database records for these monuments.\nTime took: {round(total_trvani,2)}s.'
...
def check_if_pns_in_database(database_path=str, pns_to_find=list): # Function checks if part numbers from given set already in given table in given database. Returns list list of part numbers already in database.
    ...
    query = f'SELECT "part number" FROM "part_numbers" WHERE "part number" IN ({placeholders})'
