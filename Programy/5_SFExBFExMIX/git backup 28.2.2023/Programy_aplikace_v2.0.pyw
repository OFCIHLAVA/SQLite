...
def tab_select(event): # Function to determione, which Tab of window is currently activated and set bindings accordingly
    ...
    if current_tab_index == 0:
        ...
        # bind the TAB key
        tab2_t_pn.bind_class("Text", "<Tab>", tabulator_key_binding)
        ...
        tab3_t_m.bind_class("Text", "<Key-Delete>", lambda event: tab3_clear_search_button.invoke())

        # bind the TAB key
        tab3_t_m.bind_class("Text", "<Tab>", tabulator_key_binding)

def key_bindings_by_tabs(current_tab=str):
    pass

def tabulator_key_binding(event): # This ckecks where current focus is after TAB key is pressed and according to current focus, swiches focus to next text widget. (swichich between txt widgets using TAB key)
    
    # Get the current widget in focus
    current_widget_name = event.widget.winfo_name()
    print(current_widget_name)
    # Switch focus to the other widget on respective tab
    
    # tab 2    
    # Pokud focus in search pn txt widget
    if current_widget_name == "!text":
        #print(1)
        tab2.nametowidget("!text2").focus_set()
    
    # Pokud focus in in monument txt widget
    elif current_widget_name == "!text2":
        #print(2)
        tab2.nametowidget("!text").focus_set()
    ## 
    # # tab 3      
    # # Pokud focus in monument txt widget
    # elif current_widget == tab3_t_m:
    #     tab3_t_d.focus_set()
    # # Pokud focus in description txt widget
    # elif current_widget == tab3_t_d:
    #     tab3_t_p.focus_set()
    # # Pokud focus in program txt widget
    # elif current_widget == tab3_t_p:
    #     tab3_t_sr.focus_set()
    # # Pokud focus in specific rules txt widget
    # elif current_widget == tab3_t_sr:
    #     tab3_t_m.focus_set()
...
def process_cq_file(database_path=str, cq_filepath=str): # Function to process cq txt file and add its contents into databased
    ...
    def update_progress_bar(current_value, total_value):
        progress['value'] = int(current_value / total_value * 100)
        new_window.update()  
    ...
    # Create a label to display info about actions to be performed    
    start_text = f'Vkladam data z CQ reportu do databaze\nthis may take a while...\nDO NOT CLOSE THE PROGRAM!\n\n'
    l = tk.Label(new_window,text = start_text , height=4, width=50 ,font=("Verdana", 10), padx=10, pady=10, background="black", foreground="white")
    l.pack(pady=10)
    ...
    # Run the delete function and get result message
    output_text = database_manager.update_database_bom_from_cq_file(database_path, cq_filepath, progress_callback=update_progress_bar)

    # Replace label to with Output label showing what happened    
    l.config(font=("Verdana", 10), padx=10, pady=10, background="black", foreground="white", text = output_text)    
...
def bom_process_txt_file(database_path=str, txt_filepath=str): # Function to process txt file and add its contents into database
    ...
    # Create a label to display info about actions to be performed    
    start_text = f'Vkladam data z TXT souboru do databaze\nthis may take a while...\nDO NOT CLOSE THE PROGRAM!\n\n'
    l = tk.Label(new_window,text = start_text , height=4, width=50 ,font=("Verdana", 10), padx=10, pady=10, background="black", foreground="white")
    l.pack(pady=10)
    ...
    # Run the delete function and get result message
    output_text = database_manager.update_database_bom_from_txt_list(database_path, txt_filepath, progress_callback=update_progress_bar)
...
def add_pn_monument(database_path=str, pn=str, monument=str): # Function to insert given pn and monument as pair into BOM database
    ...
    if pn and monument:
        ...
        # If record inserted succesfully, add the monument and also add part number from record into database monuments with ? as default values, if such records not already in database.
        if added_record:            
            # Check if MONUMETN record already in database
            m_already_in_db = sqllite.get_specific_record(database_path, 'boudyXprogramy', ["final monument"], [monument])            
            # If not → add monument
            if not m_already_in_db:                
            ...
            # Check if PART NUMBER record already in database
            pn_already_in_db = sqllite.get_specific_record(database_path, 'part_numbers', ["part number"], [pn])
            # If not → add part number
            if not pn_already_in_db:                
                print(f'This Part number not yet in database → inserting: {pn} with default values "?"')
                inserted_pn = sqllite.insert_record(database_path, 'part_numbers', ["part number", "description"], [pn, "?"])
                print(inserted_pn)
...
def clear_pn_in_monument_search():
    ...
    # Set focus on 1st txt widget (cursor)
    tab2.nametowidget("!text").focus_set()
...
def add_monument_record(database_path=str, monument=str, description=str, program=str, spec_rules=str): # Function to take user input for given monument and create such record in database. 
    ...
    if monument:
        # Add monument
        # If some field left blank by use, insert "?" as default values
        if not description:
            description = "?"
        if not program:
            program = "?"
...
def clear_monument_search():
    ...
    # Set focus on 1st txt widget (cursor)
    tab3.nametowidget("!text").focus_set()

def update_monument(): # Function to change data of current record 

    # Create global variable to store original monument PN. This will be needed in next function SAVE_UPDATED_MONUMENT.
    global monument_being_updated
    
    # Clean input
    monument_being_updated = tab3_t_m.get(1.0,tk.END)
    monument_being_updated = monument_being_updated.replace("\n", "").replace("\t", "")

    # Only let update, if something in monuments input window
    if monument_being_updated:
        # enable changing the monument
        tab3_t_m['state'] = 'normal'
        ...
        tab3_l_message.config(font=("Verdana", 11, "bold"), background="orange", foreground="black" ,text="↑ EDIT monument, description, program and specific rules\nfor given monument, then click SAVE changes button to commit changes. ↑")

        # hide the create (update) button
        tab3_button_update_record.config(width=6, relief="sunken", background=root['bg'], text="", bd=0,command=pass_function)

        # Change action button to show COMMIT changes for record
        tab3_button_action_record.config(width=6, font=("Verdana", 9, "bold"), background="green", foreground="white" ,text="SAVE\nchanges",padx=3 , command = lambda:save_updated_monument(monument_being_updated))        

def save_updated_monument(monument_being_updated=str): # Function to actually save the changes to database (part of update record)
    
    # insert global monument_being_updated
    #global monument_being_updated
    ...
    if monument:       
        # Check if monument we want to change to already exists
        monumet_already_exists = sqllite.get_specific_record(database_path, 'boudyXprogramy', ["final monument"], [monument])

        # Only allow updating, if such monument (one we want to update to) is not already in monuments database
        if not monumet_already_exists:
            # Actual update query using global variable (original monument) from previous function UPDATE_MONUMENT
            updated_monument = sqllite.update_record(database_path, 'boudyXprogramy', "final monument", monument_being_updated, ["final monument", "description", "program", "specific rules"], [monument, description, program, spec_rules])
            print(f'UPDATED MON {updated_monument}')
            # Also update the Monument PN in BOM database, if was changed vs original PN.
            print({monument}, {monument_being_updated})
            # update the BOM database with new monument PN if needed(not needed if no change in Monument PN)
            if updated_monument and monument != monument_being_updated:
                print(f'updatuji BOM d')
                upated_bom_monument_pn = sqllite.update_record(database_path, 'dilyXboudy', "obsazen v", monument_being_updated, ["obsazen v"],[monument])
                print(f'Also updated BOM database for monument: {monument}\nUpdatad BOM records:\n{upated_bom_monument_pn}')
            #Show info wheter something was updated
            if updated_monument:
                message = f'Monument: {monument_being_updated} updated in database to:\n{updated_monument}'
                print(message)
                ...
                tab3_l_message.config(font=("Verdana", 11, "bold"), background="yellow", foreground="black",text=message)
        # Else if that monument already exists, only allow update IF original and new monument PN are the same (only allow update for description, program, spec rules - not Monument)
        else:
            # Allow updating description, program, spec rules - not Monument
            if monument_being_updated == monument:
                # Actual update query using global variable (original monument) from previous function UPDATE_MONUMENT
                updated_monument = sqllite.update_record(database_path, 'boudyXprogramy', "final monument", monument_being_updated, ["final monument", "description", "program", "specific rules"], [monument, description, program, spec_rules])
                print(f'UPDATED MON {updated_monument}')
                #Show info wheter something was updated
                if updated_monument:
                    ...
                    message = f'Warning! Nothing updated, check why.'
                    tab3_l_message.config(font=("Verdana", 11, "bold"), background="yellow", foreground="black",text=message)  
            # Do not allow updating (would create duplicate record for 1 monument)        
            else:
                message = f'WARNING! Monument: {monument_being_updated} CANNOT be updated to: {monument}, because\nMonument: {monument} already exists. Nothing updated.'
                print(message)
                # Add Label Saying this record has been updated to database
                tab3_l_message.config(font=("Verdana", 11, "bold"), background="yellow", foreground="black",text=message)
        ...
        # Finaly delete global variable monument_being_updated (was only needed for updating)
        del monument_being_updated

def delete_monument(): # Function to delete records for given monument from database AND also records for given monument from BOM database
    ...
    if monument:
        # Delete given monument from database MONUMENTS PROGRAMS. (tab 3)
        deleted_monument = sqllite.delete_record(database_path, 'boudyXprogramy', ["final monument"], [monument])
        # Show info wheter something was deleted
        if deleted_monument:
            # Also delete BOM pairs for that monument from BOM database (tab 2)
            deleted_monument_bom_pairs = sqllite.delete_many_records(database_path, "dilyXboudy", ["obsazen v"], [(monument,)])
            if deleted_monument_bom_pairs:
                print(f'Succesfully deleted monument: {monument} from BOM database also.\nDeleted records:\n{deleted_monument_bom_pairs}')
            else:
                print(f'WARNING! No records for monument: {monument} in BOM database. Nothing deleted.')
...
def delete_many_monuments_from_txt_file(): # Function to delete many records of monuments from txt file from database  AND also records for given monuments from BOM database
    ...
    # Get filepath 
    txt_filepath = tab3_txt_delete_file_path_box.get(1.0, tk.END)
    ...
    # Create a new window to block other actions while updating database
    new_window = tk.Toplevel(root)
    new_window.title("txt file delete status")
    new_window.iconbitmap(f'{current_folder_path}\\graphics\\into db icon.ico')
    new_window.geometry("450x250")
    new_window.resizable(False, False)

    # Create a label to display info about actions to be performed    
    start_text = f'Mazu data z databaze, this may take a while...\nDO NOT CLOSE THE PROGRAM!\n\n'
    l = tk.Label(new_window,text = start_text , height=4, width=50 ,font=("Verdana", 10), padx=10, pady=10, background="black", foreground="white")
    l.pack(pady=10)
    ...
    # Run the delete function and get result message
    output_text = database_manager.delete_many_monuments(database_path, txt_filepath, progress_callback=update_progress_bar)
    ...
    tab3_button_exit.pack(pady=10)

    # Finaly clears the txt select file path window
    tab3_txt_delete_file_path_box['state'] = 'normal'
    tab3_txt_delete_file_path_box.delete(1.0, tk.END)
    tab3_txt_delete_file_path_box['state'] = 'disabled'
...
def monuments_process_txt_file(database_path=str, txt_filepath=str): # Function to process txt file and add its contents into database
    ...
    # Create a label to display info about actions to be performed    
    start_text = f'Přidávám data do databaze, this may take a while...\nDO NOT CLOSE THE PROGRAM!\n\n'
    l = tk.Label(new_window,text = start_text , height=4, width=50 ,font=("Verdana", 10), padx=10, pady=10, background="black", foreground="white")
    l.pack(pady=10)
    ...
    # Run the delete function and get result message
    output_text = database_manager.add_many_monumnets(database_path, txt_filepath, progress_callback=update_progress_bar)
...
def tab3_select_delete_txt_file():
    ...
    tab3_txt_delete_file_path_box['state'] = 'normal'
    tab3_txt_delete_file_path_box.delete(1.0, tk.END)
    tab3_txt_delete_file_path_box.insert(1.0, file_path, "center")
    tab3_txt_delete_file_path_box['state'] = 'disabled'
    tab3_txt_delete_file_path_box.tag_configure("center", justify="center")


### CREATE THE MAIN WINDOW
root = tk.Tk()
root.minsize(width=640, height=720)
...
# Add a message label
tab3_l_message= tk.Label(tab3, font=("Verdana", 11, "bold"),  background=root['bg'], text="", pady = 10)
...
'''
1. Select txt file containing Monuments to be added to MONUMENTS database and their data.
...
### C. Mass deletion monuments from TXT file

# Create label with mass deletion info

# Add txt file updates label
tab3_dl = tk.Label(tab3, font=font, text="↓ Mass MONUMENT PROGRAM DELETION from TXT file ↓", background="black", foreground="red", borderwidth=0, relief="solid", padx=3 ,pady=5)
tab3_dl.grid(row=9, rowspan=1, column=0, columnspan=7, sticky="nesw")

# Add txt file selection label
# Create a label for txt file selection
tab3_select_txt_delete_file_label = tk.Label(tab3, text=
'''
1. Select txt file containing list of Monuments to delete.
- There must be only 1 data column in file.
- Data column must contain list of monument do be deleted.
- First line of file must contain heading.
These monuments will be REMOVED from to the MONUMENT database AND BOM database.
'''
, font=font2, background="light blue", foreground="black", borderwidth=0, relief="solid", padx=10)
tab3_select_txt_delete_file_label.grid(row=10, column=0, columnspan=7, sticky="news")

# Create button to select txt filepath
tab3_txt_file_delete_icon = tk.PhotoImage(file=f'{current_folder_path}\\graphics\\txt_openfile.png')
tab3_select_txt_delete_button = tk.Button(tab3, image=tab3_txt_file_icon, background="gray", command=tab3_select_delete_txt_file)
tab3_select_txt_delete_button.grid(row=11, column=0, sticky="w")

# Create text box to show path to txt file
tab3_txt_delete_file_path_box = tk.Text(tab3, wrap="none", height=1  ,borderwidth=1, font=font2, background="dark gray", foreground="black", relief="solid", padx=5)
tab3_txt_delete_file_path_box['state'] = 'disabled'
tab3_txt_delete_file_path_box.grid(row=11, column=0, columnspan=7, sticky="nsew", padx=(30,0))

# Create button to process the selected file
tab3_txt_delete_icon = tk.PhotoImage(file=f'{current_folder_path}\\graphics\\delete from database.png')
tab3_process_txt_delete_button = tk.Button(tab3, image=tab3_txt_delete_icon, background="gray", command=delete_many_monuments_from_txt_file)
#select_cq_button = tk.Button(tab2, text="↓ SELECT FILE ↓", command=select_cq_file, font=("Verdana", 11, "bold"), background="gray", foreground="white")
tab3_process_txt_delete_button.grid(row=12, column=2, sticky="e")
