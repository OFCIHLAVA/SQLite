...
### BOM preparation stuff

with open(f'{current_folder_path}\\3 sql dil x boudy\\kusovniky_update.txt', 'r', encoding='UTF-8') as f:
    update_file_data = [[field.strip() for field in line.replace("|","").split()] for line in f if line and line[0] == "|"]
    # data = [[field.strip() for field in line.split("\t")] for line in data if line[0] == "|"]
    f.close()

pridat_do_databaze = sqllite.get_pn_in_monuments(update_file_data)
