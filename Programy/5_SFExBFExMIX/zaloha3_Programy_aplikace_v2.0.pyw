...
def add_pn_monument(database_path=str, pn=str, monument=str): # Function to insert given pn and monument as pair into BOM database
    ...
    if pn and monument:
        ...
        if added_record:            
            ...
            pn_already_in_db = sqllite.get_specific_record(database_path, 'part_numbers', ["part numbers"], [pn])
...
def update_monument(): # Function to change data of current record 
    ...
    monument_being_updated = monument_being_updated .replace(" ","").replace("\n", "").replace("\t", "")
...
def delete_many_monuments_from_txt_file(): # Function to delete many records of monuments from txt file from database  AND also records for given monuments from BOM database
   ...
   # helper function to close prompt window
    ...
    txt = tk.Text(new_window, height=5, width=50 ,font=("Verdana", 10), padx=10, pady=10)
    txt.pack()
    start_text = f'Mazu dataz databaze, this may take a while...\nDO NOT CLOSE THE PROGRAM!\n\n'
    txt.tag_configure("center", justify="center")
    txt.insert(tk.END, start_text, "center")
    txt['state'] = "disabled"
    time.sleep(1)
    ...
    txt['state'] = "normal"
    txt.delete(1.0, tk.END)
    output_text = database_manager.delete_many_monuments(database_path, txt_filepath)
    print(output_text)
    txt.insert(tk.END, output_text, "center")
    txt['state'] = "disabled"
